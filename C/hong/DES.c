...
//CBC模式下的DES加密函数
void des_CBC_enc(FILE *inpFile,FILE *outFile,const_DES_cblock IV){
	const_DES_cblock iv ;
...
//CBC模式下的DES解密函数
void des_CBC_dec(FILE *inpFile,FILE *outFile,const_DES_cblock IV){
	const_DES_cblock iv ;
	copyValue(IV,iv,sizeof(const_DES_cblock));
...
int main(int argc, char** argv)
{
    if(argc != 5) {
		...
		*/
        ...
        if(inpFile && outFile) {
			printf("加密文件创建成功！\n");
			des_CBC_enc(inpFile,outFile,IV);
        } else {
            printf("打开文件失败！\n");
        }
        fclose(inpFile);
		...
		FILE *decFile = fopen("DES_dec.txt", "wb");
		if(incFile && decFile) {
			printf("解密文件创建成功！\n");
			des_CBC_dec(incFile,decFile,IV);
        } else {
            printf("打开文件失败！\n");
        }
		fclose(incFile);
